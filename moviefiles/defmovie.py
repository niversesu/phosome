...
def makevideo(filename, musicadd=False, watermarkadd=False, watermarkopacity=0.5, video_volume=1.0, music_volume=1.0):
    print(f"Filename: {filename}, Music add: {musicadd}")
    time.sleep(0.5)
    print(f"Watermark add: {watermarkadd}, Watermark opacity: {watermarkopacity}")
    time.sleep(0.5)
    ...
    file_extensions = "mp4", "mkv", "mp3", "wav"
    picture_path = os.path.join("assets/pictures", "watermark.png")
    ...
    video_files = [file for file in os.listdir(folder) if file.endswith((file_extensions))]
...
if __name__ == "__main__":
    ...
    makevideo(filename, musicadd=True)  # Pass the boolean value as True
