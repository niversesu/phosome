...
def create_video():
    random_combination = checkbox_var4.get()
    ...
    try:
        ...
        music_volume = float(entry3.get()) if entry3.get() and float(entry3.get()) <= 1.0 else 0.15
        watermarkopacity = float(entry4.get()) if entry4.get() and float(entry4.get()) <= 1.0 else 0.15
        makevideo(filename, random_combination, musicadd, watermarkadd, watermarkopacity, video_volume, music_volume)
    except:
...
checkbox_create4 = CustomCheckBox(
    master=frame,
    text="Random Combination?",
